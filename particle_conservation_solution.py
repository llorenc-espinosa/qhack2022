...
def binary_list(m, n):
    ...
    arr = [(m//2**(i-1))%2 for i in range(n,0,-1)]
    # QHACK #
...
def is_particle_preserving(circuit, n):
    ...
    for state_before in states:
        hw_before = sum(state_before)
        state_after_amp = circuit(state_before)
        state_after = []
        for i in range(len(state_after_amp)):
            if state_after_amp[i] != 0+0j:
                state_after.append(binary_list(i,n))
        hw_after = []
        for state in state_after:
            hw_after = sum(state)
