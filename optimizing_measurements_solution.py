...
def check_simplification(op1, op2):
    ...
    # QHACK
    for qubit in range(len(op1)):
        if (op1[qubit] != op2[qubit]) and (op1[qubit]!="I" and op2[qubit]!="I"):
            return False
    return True
    # QHACK
...
def join_operators(op1, op2):
    ...
    for qubit in range(len(op1)):
        if op1[qubit]=="I":
            arr.append(op2[qubit])
        else: arr.append(op1[qubit])
    return arr
    # QHACK
...
def compression_ratio(obs_hamiltonian, final_solution):
    ...
    num = 0
    num_com = 0
    for obs in obs_hamiltonian:
        if obs != "I":
            num += 1
    for obs in final_solution:
        if obs != "I":
            num_com += 1

    return 1 - num_com/num
